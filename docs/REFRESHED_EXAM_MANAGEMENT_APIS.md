...
### Multiple Classes Selection Workflow

#### Step 1: Exam Details Form
Admin enters basic exam information:
- **Exam Name**: "Mid Term Examination 2026"
- **Exam Type**: "Mid Term"
- **Subjects**: Select and assign max marks for each subject (e.g., Math: 100, Science: 50)

#### Step 2: Class Selection Mode Toggle
Frontend provides two options:

**Option A: Single Class (Legacy Mode - Backward Compatible)**
- Select one class
- Select sections under that class
- Submit directly

**Option B: Bulk Multiple Classes (Recommended for Creating Same Exam Across Multiple Classes)**
- Toggle "Create for Multiple Classes" checkbox
- Appears with "Add Class" button
- Each class row shows:
  - **Class Dropdown** (searchable)
  - **Sections Checkboxes** (auto-populates when class selected)
  - **Remove Button** (to delete that class from bulk list)

#### Step 3: Class & Section Selection (Bulk Mode)

When admin clicks "Add Class":
...
When "Create Exam" is clicked, the form constructs the payload:
...
1. **Class Uniqueness**: A class cannot appear twice in the targets list. If user tries to add the same class again, show error: "This class is already added."

2. **Section Validation**: When class is selected, auto-populate only sections belonging to that class.

3. **Bulk Limit** (Optional): Consider limiting to 10 classes per request to avoid overload. Show warning if user adds more than 10 classes.

4. **Subject Persistence**: Subjects are applied to all exams being created. Frontend should clearly display: "The same subjects and marks will be used for all selected classes."

5. **Success Response**: After creation, show:
   - ✅ "Successfully created 3 exams for Class 10, Class 9, and Class 8"
   - List the created exams with their class names

6. **Error Handling**:
   - If any section doesn't belong to its class: "Section X does not belong to Class Y"
   - If exam name already exists for a class: "Exam 'Mid Term Examination 2026' already exists for Class 10"
